...
const helper = {
  getWinWH() {
    return {
      w: (process.stdout as any)?.columns,
      h: (process.stdout as any)?.raws,
    };
  },
  ...
  hr(inputHrStr: string, length?: number) {
    length = length || helper.getWinWH().w;
...
export class TerminalRenderer extends Renderer {
  ...
  constructor(options?: TerminalRendererOptions) {
    ...
    if (!options?.width && helper.getWinWH().w) {
      opts.width = options?.width || helper.getWinWH().w;
    }
    super(opts);
